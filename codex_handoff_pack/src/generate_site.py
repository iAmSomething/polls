...
def render_html(
    ...
    swing = 0.0
    if ranking_rows:
        swing = sum(abs(float(r["delta"])) for r in ranking_rows) / len(ranking_rows)
    ...
    for _, r in weights_df.iterrows():
        ...
        weight_rows.append(
            ...
            <tr>
              ...
              <td>
                <div class="wbar-wrap">
                  <div class="wbar" style="width:{wp:.2f}%"></div>
                </div>
...
<body>
  <div class="wrap">
    ...
    <section class="main-grid">
      ...
      <aside class="panel">
        <div class="panel-title" style="margin-bottom:8px;">예측 랭킹</div>
        <div class="rank-wrap">
          {''.join(ranking_html)}
        </div>
      </aside>
    ...
    <section class="news">
      <div class="panel-title" style="margin: 0 0 8px;">최근 여론조사 기사 링크</div>
      <div class="news-grid">
        {''.join(article_cards)}
      </div>
    </section>
    ...
    <section class="method">
      <details>
        ...
        <p class="method-p">
          2023년부터 2025년 6월 선거까지, 여론조사기관의 정당지지율과 실제 선거결과를 비교해 정확도(MAE)를 산출했습니다.
          이후 정확도 상위 클러스터(9개 기관)만 사용해 합성 시계열을 만들고, 기관별 가중치는 1/MAE를 정규화해 적용합니다.
          주간 업데이트에서는 Huber 손실 기반으로 가중치 안정성을 유지하도록 설계했습니다.
        </p>
        <table>
  ...
  <script>
    ...
    function buildTraces() {{
      ...
      tracesData.forEach((p) => {{
        out.push({{
          ...
          legendgroup: p.party, line: {{ color: p.color, width: 2.7 }}, hovertemplate: "%{{x}}<br>%{{fullData.name}}: %{{y:.2f}}%<extra></extra>"
        }});
        out.push({{
        ...
        out.push({{
          ...
          text: ["예측치"], textposition: "middle right",
    ...
    function setBtnActive(key) {{
      fbtns.forEach((b) => b.classList.toggle("active", b.dataset.range === key));
    }}
    function endDate() {{
      let d = null;
      tracesData.forEach((p) => {{
